...
def Grad_CAM_MAP():
    test_file_path = 'test_CAM/0.png'
    CHECK_POINT_PATH = './multiscale_286_images_40_2/FCNnet_model.ckpt'
    train_batch = 1
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1):
            ...
            # img_org, label_one_hot, label = sess.run(
            #     [train_images, train_label_one_hot, train_labels])
            img_org = cv2.imread(test_file_path, 0)
            img_oo= cv2.imread(test_file_path)
            batch_img = get_VGG_input(img_org, train_batch)
            label_one_hot = [[1,0,0,0,0]]


            convlustion_map = sess.run(cam,
                                                     feed_dict={y_ : label_one_hot, images1: batch_img, keep_f: 1.0, is_training: False})


            print(convlustion_map.shape)
            # convlustion_map
            heatmap = convlustion_map / np.max(convlustion_map)
            heatmap = cv2.resize(heatmap, (224, 224))

            # heatmap = image /np.max(image)
            heatmap = np.uint8(255*heatmap)
            heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
            cv2.imwrite('CAM/Grad_CAM_0.jpg', heatmap)
            superimposed_img = heatmap * 0.4 + img_oo
            superimposed_img = np.clip(superimposed_img, 0, 255)
            superimposed_img = superimposed_img.astype(np.uint8)
            cv2.imwrite('CAM/org_map_0.jpg', superimposed_img)
