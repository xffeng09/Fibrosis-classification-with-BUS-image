...
#         print('aaaaaaaaaaaaaaa',U_batch.shape)
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(number_of_test):
            ...
            fc, acc = sess.run([fc4, accuracy], feed_dict={y_ : label, images1: batch_img, keep_f: 1.0, is_training: False})
#            print("CCCCCCCCCCCCC",fc)
            cf_softmax = softmax_(fc)
            if epoch == 0:
                y_pred = cf_softmax
                tru_label = label
            else:
                y_pred = np.concatenate((y_pred, cf_softmax),axis = 0)
                tru_label = np.concatenate((tru_label, label),axis = 0)
                print("trulabelllllll", y_pred)
                print("trulabe44444", tru_label)
  
        np.savetxt("proposed_prediction_4.csv", y_pred,fmt="%f",delimiter=",")
        np.savetxt("proposed_true_label_4.csv", tru_label,fmt="%d",delimiter=",")
        coord.request_stop()
        coord.join(threads) 
    print("Done testing")        
