...
def test_evaluation():
    train_file_path = '../liver_test_fivefold_1.tfrecords'
    CHECK_POINT_PATH = './proposed_model_fivefold_1/FCNnet_model.ckpt'
    train_batch = 1

    train_image, train_label = read_and_decode(train_file_path)
    train_images, train_labels,train_labels1 = CreateBatch_list([ train_image, train_label],train_batch)
...
#         print('aaaaaaaaaaaaaaa',U_batch.shape)
    ...
    with tf.Session() as sess:
        ...
        test_saver.restore(sess,save_path=CHECK_POINT_PATH)
        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(coord = coord)
        accuracy_test = 0
        number_of_test = 6000
        num=0
        for epoch in range(number_of_test):
            print('epoch number:', epoch)
            img, label,label1 = sess.run([train_images, train_labels,train_labels1])
            batch_img = get_VGG_input(img, train_batch)
            
            
            y_index, acc,cf_softmax = sess.run([predictions, accuracy,soft_fc], feed_dict={y_ : label, images1: batch_img, keep_f: 1.0, is_training: False})
            
             
            
            
            
            accuracy_test = accuracy_test + acc
            num +=1
            # cf_softmax = softmax_(fc)
            ...
            if epoch == 0:
                y_pred = y_index
                tru_label = label1
                tru_label_onehot = label
                y_pred2 = cf_softmax
            else:
                y_pred2 = np.concatenate((y_pred2, cf_softmax), axis=0)
                y_pred = np.concatenate((y_pred, y_index),axis = 0)
                tru_label = np.concatenate((tru_label, label1),axis = 0)
                tru_label_onehot = np.concatenate((tru_label_onehot, label), axis=0)
                print("trulabelllllll", y_pred)
                print("trulabe44444", tru_label)
           
        print('accuracy:', accuracy_test/num)
        print("shape----", y_pred.shape)
        print("shape----", tru_label.shape)
        print("shape----", tru_label_onehot.shape)
        AUC = metrics.roc_auc_score(tru_label_onehot, y_pred2, average='micro')
        f1 = f1_score( tru_label, y_pred, average='macro' )
        Pre = precision_score(tru_label, y_pred, average='macro')
        Rec = recall_score(tru_label, y_pred, average='macro')
        acc = accuracy_score(tru_label, y_pred)
        CMC=confusion_matrix(tru_label, y_pred)
        print('AUC', AUC)
        print("F1-score", f1)
        print("precision", Pre)
        print("recall_score", Rec)
        print("ACC", acc)
        a = [Pre, Rec, f1, acc, AUC]
        np.savetxt("proposed_metrics_fivefold_1.csv", a, fmt="%f", delimiter=",")
        np.savetxt("proposed_preddict_fivefold_1.csv", y_pred2, fmt="%f", delimiter=",")
        np.savetxt("proposed_true_label_fivefold_1.csv", tru_label_onehot, fmt="%d", delimiter=",")
        np.savetxt("CMC_Proposed_fivefold_1.csv", CMC, fmt="%d", delimiter=",")
        coord.request_stop()
        coord.join(threads) 
    print("Done testing")     
